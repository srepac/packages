...
# 3. Update /usr/lib/systemd/system/kvmd-oled.service file
"""
Change this line FROM:

ExecStart=/usr/bin/kvmd-oled --height=32 --offset-y=6 --clear-on-exit

TO

ExecStart=/usr/bin/srepac-oled --height=32
"""
# 4. Enable kvmd-oled services via "systemctl enable --now kvmd-oled" (applies to both v2/v3)
# 5. reboot on v2 and watch the magic on your oled screen; no need to reboot v3
###
import sys
import socket
import logging
        ...
        while True:
            with canvas(device) as draw:
                ...
                if rem == 0:   ### first page is fqdn, model number, image/kvmd version (v2-hdmi, v2-hdmiusb, etc...)
                    ...
                    connstatus = os.popen(" count=0; ethip=$( ip -o a show | grep -v inet6 | awk '{print $2, $4}' | cut -d'/'  -f1 | egrep 'br|eth' ); ssid=$( iwconfig wlan0 | grep SSID | cut -d'\"' -f2 ) ; if [[ \"$ethip\" == \"\" ]]; then echo \"LAN not connected\"; else printf \"%s %s\n\" $ethip; count+=1; fi; if [[ \"$ssid\" == \"\" ]]; then echo \"Wifi not connected\" ; else echo \"SSID $ssid\"; count+=1; fi ; if [[ $count -gt 0 ]]; then echo $( ip -o a | grep tailscale | awk '{print $2, $4}' | cut -d'/' -f1 | grep -v inet6 | sed 's/tailscale/ts/g' ); fi ")
                    ...
                    x = os.popen(" for i in `mount | egrep 'sda|mmc' | awk '{print $1}' | sort | grep -v p1`; do echo -n `df -h $i | grep -v Filesystem | sort | awk '{print $1, $5, $4}' | sed -e 's+/dev/mmcblk0++' -e 's/p3/msd/g' -e 's+/dev/sda1+msd+g' -e 's+p2+/+g' -e 's+p1+/boot+g'`' '; mount | grep $i | awk '{print $NF}' | awk -F, '{print $1}' | sed 's/(//g'; done ")
                    sdcard = x.read()
                    text = f"Temp {temps}\n{sdcard}"
                screen += 1
                draw.multiline_text((0, 0), text, font=font, fill="white")
